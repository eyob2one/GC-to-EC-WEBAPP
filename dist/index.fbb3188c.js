...
function ethTime(date, mon, yr, hr, min, sec) {
    if (date === 31) this.date = 30; //todo
    else this.date = date;
    if (yr > 200) this.year = yr;
    else this.year = yr + 1900;
    this.month = mon;
    this.hour = hr;
    this.minute = min;
    this.second = sec;
    ...
    if (hr < 13) this.timeString = leftpad(hr) + ":" + leftpad(min) + ":" + leftpad(sec) + " a.m.";
    else this.timeString = leftpad(hr - 12) + ":" + leftpad(min) + ":" + leftpad(sec) + " p.m.";
...
function toEthiopianDateTime(eurDate1) {
    var difference = eurDate1.getTime() - new Date("September 12, 1971").getTime();
    ...
    var remainingHours = eurDate1.getHours(); // - 6 to account for traditional local time
    if (remainingHours < 0) remainingHours += 24;
    var ethDate1 = new ethTime(remainingDays + 1, remainingMonths + 1, remainingYears + 4 * fourYearsPassed + 1964, remainingHours, eurDate1.getMinutes(), eurDate1.getSeconds());
    return ethDate1;
}
function toEthiopianDateTimeString(eurDate1) {
    return toEthiopianDateTime(eurDate1).dateTimeString;
}
function toEthiopianDateString(eurDate1) {
    return toEthiopianDateTime(eurDate1).dateString;
}
function toEuropeanDate(ethDate1) {
    var initialEuropean = new Date(new Date(ethDate1.year, ethDate1.month - 1, ethDate1.date).getTime() + globalDifference);
    if (ethDate1.month === 13) {
        if (ethDate1.year % 4 === 3) maxDate = 6;
        else maxDate = 5;
        if (ethDate1.date > maxDate) {
            errMsg = "Pagume Only has " + maxDate + " days at year " + ethDate1.year + ". Please select another day.";
    ...
    for(var count = -8; count < 9; count++){
        ...
        var remainingMonths = Math.floor((difference - fourYearsPassed * fourYears - remainingYears * oneYear) / (30 * oneDay));
        var remainingDays = Math.floor((difference - fourYearsPassed * fourYears - remainingYears * oneYear - remainingMonths * 30 * oneDay) / oneDay);
        if (ethDate1.date === remainingDays + 1 && ethDate1.month === remainingMonths + 1) return EngDate;
    }
}
function toEuropeanDateString(ethDate1) {
    EuropeanDate = toEuropeanDate(ethDate1);
...
},{}]},["kzbYw","bDbGG"], "bDbGG", "parcelRequire94c2")
